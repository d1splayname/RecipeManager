...
const Home = () => {
	...
	async function ScrapeWebsiteBackup(website: string) {
		const prompt: string =
			`Extract recipe name, recipe category, ingredients, and steps from this website.
			Do not return anything else except an array as follows [recipe name, recipe category, [ingredients], [steps]].
			Website HTML is below:
		`;

		const res = await fetch(`/api/ollamaPrompt`, {
			method: "POST",
			headers: {"Content-Type": "application/json"},
			body: JSON.stringify({
				model: "gemma3:4b",
				prompt: prompt + website,
				stream: false
			})
		});
		...
		console.log("🚀 ~ scrapeWebsite ~ parsedRes:", parsedRes)
		console.log("🚀 ~ scrapeWebsite ~ parsedRes:", parsedRes["response"])
		console.log("🚀 ~ scrapeWebsite ~ parsedRes:", parsedRes["done_reason"])


		// setQueryResult(res.json());
	}
	...
	useEffect(() => {
		...
		FetchWebsite("53", "https://ninjatestkitchen.eu/recipe/vanilla-bean-gelato/");

		// 	for (const recipe of recipes) {
		// 		console.log("🚀 ~ App ~ recipe:", recipe);
		// 		console.log(`🚀 ~ App ~ recipe["id"]:`, recipe["id"]);
		// 		console.log(`🚀 ~ App ~ recipe["url"]:`, recipe["url"]);

		// 		FetchWebsite(recipe["id"], recipe["url"]);
		// 	}
	}, [recipes]);
